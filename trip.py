...
class Trip():
    ...
    def __init__(self, departure: City) -> None:
        ...
        self.cities_to_go = []
        self.cities_to_go.append(departure)

    def add_next_city(self, city: City) -> list[City]:
        """
        Adds the next city to this trip.
        """
        self.cities_to_go.append(city)
        return self.cities_to_go
    ...
    def total_travel_time(self, vehicle: Vehicle) -> float:
        ...
        """
        total_travel_time_var = 0

        for i in range(len(self.cities_to_go) - 1):
            departure = self.cities_to_go[i]
            arrival = self.cities_to_go[i+1]
            total_travel_time_var += vehicle.compute_travel_time(departure, arrival)
            if total_travel_time_var == math.inf:
                return math.inf
        return total_travel_time_var
    ...
    def find_fastest_vehicle(self, vehicles: list[Vehicle]) -> (Vehicle, float):
        ...
        shortest_time = self.total_travel_time(vehicles[0])

        for vehicle in vehicles:
            travel_time_for_each_vehicle = self.total_travel_time(vehicle)
            if travel_time_for_each_vehicle < shortest_time:
                shortest_time = travel_time_for_each_vehicle
                fastest_vehicle = vehicle

        if shortest_time == math.inf:
            return None, math.inf

        return fastest_vehicle, shortest_time
    ...
    def __str__(self) -> str:
        ...
        """
        city_order = str(self.departure)
        for city in self.cities_to_go[1:]:
            city_order += " -> " + str(city)
        return city_order
